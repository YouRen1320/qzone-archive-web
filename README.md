...
[在线体验](https://qzone.iyouren.top/)（归档真实数据前，请先阅读下方的运行边界与风险）

![拾光册线上首页：雨窗前的临时归档入口](docs/images/product-homepage.jpg)

_2026 年 9 月 7 日摄于线上首页；画面不包含 QQ 号、登录二维码、任务令牌或归档内容。_
...
- **一次一人**：初始部署默认只运行一个归档任务，其他任务按先后顺序排队并显示前方人数。
- **Cookie 不落盘**：QQ 登录 Cookie 只存在于 Rust 进程内存，不返回前端、不写日志、不进入导出包。
- **任务级临时 SQLite**：每页事务提交、去重和断点都在当前用户的临时文件中完成。
- **用完即删**：未开始的任务 20 分钟后清理，完成或暂停的数据保留 2 小时；下载后缩短为 10 分钟，也支持立即删除。
- **占位有上限**：连续 5 分钟没有新进度或单次运行满 60 分钟会自动暂停，保留断点并立即让出执行位。
- **导出可带走**：ZIP 内含离线查看器、原始 JSONL、SQLite、清单，以及成功下载的媒体文件。
- **完成即刻翻阅**：恢复完成后，当前页面自动进入记录；搜索、年份和分类均按页读取，不必先下载。
...
恢复完成后，当前网页会自动打开记录。ZIP 是可长期保存的完整备份；其中仍保留 `index.html` 与 `data.js`，方便电脑端离线查看和兼容既有导出结构，但本站不再提供“打开已有 ZIP”入口。
...
测试覆盖范围、当前核验结果和仍需人工执行的上线门禁见[验证说明](docs/testing.md)。自动化测试不会请求 QQ 接口，也不包含真实账号数据。

## 运行边界与风险

- QQ 可能限制云服务器 IP、返回 HTTP 500、触发风控或改变非公开接口。
- 页面可以关闭，排队和运行会在服务器继续；自动暂停后需重新扫码，才会重新加入队列并从断点继续。
